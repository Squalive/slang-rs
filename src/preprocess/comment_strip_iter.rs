...
static AC: Lazy<AhoCorasick> =
    Lazy::new(|| AhoCorasick::new(PATTERNS).expect("Failed to build AhoCorasick"));
...
";
        ...
        for (i, (processed_line, original_line)) in
            processed.iter().zip(original.iter()).enumerate()
        {
            assert_eq!(
                processed_line.len(),
                original_line.len(),
    ...
    #[test]
    fn test_fast_path() {
        let simple_lines = ["just some code", "let x = 5;", "fn main() {}"];
