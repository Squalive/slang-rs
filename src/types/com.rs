use crate::Uuid;
use crate::{Blob, Interface, Result, Unknown};
use alloc::boxed::Box;
use alloc::sync::Arc;
use alloc::vec::Vec;
...
impl<T: ISlangUnknown> Com<T> {
	...
	pub fn new_unknown(value: T) -> Self {
		Self::new(&Self::UNKNOWN_VTABLE as *const sys::ISlangUnknown__bindgen_vtable as _, value)
...
impl<T: ISlangCastable> Com<T> {
	...
	pub fn new_castable(value: T) -> Self {
		Self::new(&Self::CASTABLE_VTABLE as *const sys::ISlangCastable_vtable as _, value)
...
impl<T: ISlangBlob> Com<T> {
	...
	pub fn new_blob(value: T) -> Self {
		Self::new(&Self::BLOB_VTABLE as *const sys::ISlangBlob_vtable as _, value)
		...
		Ok(path_str) => {
			...
			match com.value.load_file(path, &mut buf) {
				Ok(_) => unsafe {
					...
					let vtable = &*((&*blob).vtable.as_ptr() as *const sys::ISlangUnknown__bindgen_vtable);
...
impl<T: ISlangFileSystem> Com<T> {
	...
	pub fn new_file_system(value: T) -> Self {
		Self::new(&Self::FILE_SYSTEM_VTABLE as *const sys::ISlangFileSystem_vtable as _, value)
