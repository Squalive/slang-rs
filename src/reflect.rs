...
impl EntryPoint {
	...
	pub fn compute_thread_group_size(&self) -> [u64; 3] {
		let mut out = [0; 3];
		rcall!(spReflectionEntryPoint_getComputeThreadGroupSize(self, 3, &mut out as *mut u64));
		out
	}
...
impl VariableLayout {
	...
	pub fn category_by_index(&self, index: u32) -> ParameterCategory {
		self.type_layout().map_or(ParameterCategory::None, |tl| tl.category_by_index(index))
	}
...
mod type_layout {
	...
	impl TypeLayout {
		...
		pub fn kind(&self) -> Option<TypeLayoutKind<'_>> {
			...
			match kind {
				...
				S::ConstantBuffer => Some(L::SingleElementContainer(ContainerTypeLayout(self), SingleElementContainerType::ConstantBuffer)),
				S::ParameterBlock => Some(L::SingleElementContainer(ContainerTypeLayout(self), SingleElementContainerType::ParameterBlock)),
				S::TextureBuffer => Some(L::SingleElementContainer(ContainerTypeLayout(self), SingleElementContainerType::TextureBuffer)),
				S::ShaderStorageBuffer => Some(L::SingleElementContainer(ContainerTypeLayout(self), SingleElementContainerType::ShaderStorageBuffer)),
	...
	impl<'a> StructTypeLayout<'a> {
		...
		pub fn field_by_index(self, index: usize) -> Option<&'a VariableLayout> {
			rcall!(spReflectionTypeLayout_GetFieldByIndex(self.0, index as _) as Option<&VariableLayout>)
		}
		...
		pub fn find_field_index_by_name(self, name: &str) -> usize {
			...
			rcall!(spReflectionTypeLayout_findFieldIndexByName(self.0, start as *const _,end as *const _)) as _
		}
	}
